...
WORKFLOW_ARGS = ["--initial-active-space-solver", "macis_asci", "--autocas", "--max-determinants", "100"]
...
def parse_log(text: str) -> dict:
    ...
    if m := re.search(r"Nuclear Repulsion Energy: ([\d.]+) Eh", text):
        data.setdefault("structure", {})["nuclear_repulsion_energy_eh"] = float(m.group(1))
    ...
    # Orbitals Summary
    orb = {}
    if m := re.search(r"AOs: (\d+)", text): orb["aos"] = int(m.group(1))
    if m := re.search(r"MOs: (\d+)", text): orb["mos"] = int(m.group(1))
    if m := re.search(r"Type: (\w+)", text): orb["type"] = m.group(1)
    ...
    if orb: data["orbitals_summary"] = orb
    ...
    hams = []
    for m in re.finditer(r"Active Orbitals: (\d+)\s+Total Orbitals: (\d+)\s+Core Energy: ([-\d.]+)", text):
        hams.append({"active_orbitals": int(m.group(1)), "total_orbitals": int(m.group(2)), "core_energy": float(m.group(3))})
    if hams: data["hamiltonian_summaries"] = hams
    
    # Sparse CI finder
    if m := re.search(r"Sparse CI finder \((\d+) dets\) = ([-\d.]+) Hartree \(ΔE = ([\d.]+) mHartree\)", text):
        sci = {"n_dets": int(m.group(1)), "energy_hartree": float(m.group(2)), "delta_e_mhartree": float(m.group(3))}
        # Parse determinants
        if dm := re.search(r"Stored wavefunction with \d+ determinants\s*\nDeterminants:\s*\n((?:  .+\n)+)", text):
            dets = []
...
def run_workflow(xyz, log_path, script):
    ...
    with open(log_path, "w") as f:
        f.write(f"=== WORKFLOW START ===\nStart: {t0.isoformat()}\nCommand: {' '.join(str(c) for c in cmd)}\n\n")
        r = subprocess.run(cmd, stdout=f, stderr=subprocess.STDOUT)
        f.write(f"\n\n=== WORKFLOW END ===\nEnd: {datetime.now().isoformat()}\nElapsed seconds: {(datetime.now() - t0).total_seconds():.2f}\n")
    
    print(f"{'OK' if r.returncode == 0 else 'FAILED'} ({(datetime.now() - t0).total_seconds():.0f}s)")
...
def main():
    ...
    p.add_argument("--collate-only", action="store_true", help="Just re-parse existing raw_output/ logs")
    ...
    if args.images_only:
        if not HAS_RDKIT: sys.exit("rdkit required for --images-only")
        for xyz in sorted(xyz_dir.glob("*.xyz")):
            print(f"  {xyz.stem}...", end=" ")
            print("OK" if xyz_to_image(xyz, here / "images" / f"{xyz.stem}.png") else "FAILED")
        return
    ...
    if args.molecules: xyzs = [x for x in xyzs if x.stem in args.molecules]
